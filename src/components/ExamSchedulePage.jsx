  ...
  return (
    <div className="">
      ...
      {group.questions?.map((question, qIndex) => {
        if (question.type === "alternative") {
          return question.questions?.map((altQuestion, altIndex) => (
            <React.Fragment key={`${qIndex}-${altIndex}`}>
              <QuestionRow question={altQuestion} parentQuestion={question} />
              {altIndex === 0 && (
        ...
        return <QuestionRow key={qIndex} question={question} />;
      })}
    </div>
  );
};
  ...
  return (
    <div className="grid grid-cols-7 text-center border-black">
      <p className="border border-black">
        {parentQuestion
          ? `${parentQuestion.number}${question.number.slice(-3)}`
          : question.number}
      </p>
...
const ExamSchedulePage = ({ data }) => {
  ...
  return (
    <div className="relative">
      ...
      <div
        id="pdf-content"
        className="bg-white p-8 text-black max-w-5xl mx-auto mb-20 mt-10 playfair-display-bold"
      >
        {/* Content as defined in the original component */}
        <div className="overflow-hidden mt-14">
          <div className="grid grid-cols-4 w-full text-base mb-10 border-[1px] border-black">
            ...
            <div className="grid grid-rows-3 border-l-[1px] border-black col-span-3 text-center border-[1px] h-24">
              <h1 className="text-3xl font-bold uppercase">{universityName}</h1>
              <p className="text-lg uppercase font-bold">{examDetails.title}</p>
              <p className="text-sm">{examDetails.session}</p>
            </div>
            ...
            />
            <InfoRow label="Semester" value={courseInfo.semester} />
            <InfoRow label="Course/Subject Name" value={courseInfo.name} />
            <InfoRow label="Course/Subject Code" value={courseInfo.code} />
            <InfoRow label="Maximum Marks" value={examParameters.maxMarks} />
            <InfoRow label="Time Duration" value={examParameters.duration} />
            <InfoRow
      ...
      <button
        ...
        className="z-50 fixed bottom-8 right-8 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-full shadow-lg transition-transform transform hover:scale-105"
      >
        Generate PDF
      </button>
    </div>
